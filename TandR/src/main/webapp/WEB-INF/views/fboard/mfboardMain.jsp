		...
		<div id="maincontent">
			...
			<div id="contentBox">
				...
				<div id="numberbar">
					...
					<button onclick="location.href='masterFboard.fb?pageNum=${startPage-1}'">이전</button>
				</c:if>
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<c:if test="${currentPage== i }">
						<button onclick="location.href='masterFboard.fb?pageNum=${i}'" disabled="disabled">${i}</button>
					</c:if>
					<c:if test="${currentPage!= i }">
						<button onclick="location.href='masterFboard.fb?pageNum=${i}'">${i}</button>
					</c:if>
				</c:forEach>
				<!-- 보여줄 것이 남아있다 -->
				<c:if test="${endPage < totalPage }">
					<button onclick="location.href='masterFboard.fb?pageNum=${endPage+1}'">다음</button>
				</c:if>
				</div>
				<div id="searchbox">
