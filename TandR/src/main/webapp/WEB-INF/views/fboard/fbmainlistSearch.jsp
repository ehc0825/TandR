...
<div id="maincontent">
		...
		<div id="main_container">
			...
			<br>
			<div id="write_btn">
				<button id="writecontent" onclick="location.href='fboardWriteForm.fb?fbNum=0&pageNum=${currentPage}'">글쓰기</button>
				</div>
			<div id="pagebar" align="center">
				<c:if test="${startPage > PAGE_PER_BLOCK }">
					<button onclick="location.href='searchFbmainlist.fb?pageNum=${startPage-1}&fbsearch=${fbsearch }&whereFb=${whereFb}'">이전</button>
				</c:if>
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<c:if test="${currentPage== i }">
						<button onclick="location.href='searchFbmainlist.fb?pageNum=${i}&fbsearch=${fbsearch }&whereFb=${whereFb}'" disabled="disabled">${i}</button>
					</c:if>
					<c:if test="${currentPage!= i }">
						<button onclick="location.href='searchFbmainlist.fb?pageNum=${i}&fbsearch=${fbsearch }&whereFb=${whereFb}'">${i}</button>
					</c:if>
				</c:forEach>
				<!-- 보여줄 것이 남아있다 -->
				<c:if test="${endPage < totalPage }">
					<button onclick="location.href='searchFbmainlist.fb?pageNum=${endPage+1}&fbsearch=${fbsearch }&whereFb=${whereFb}'">다음</button>
				</c:if>
			<!-- num=0 처음 쓴글 -->
		</div>
		</div>
