		...
		<div id="maincontent">
			...
			<div id="contentBox">
				...
				<div id="numberbar">
					...
					<button onclick="location.href='searchRvlist.re?pageNum=${startPage-1}&rvsearch=${rvsearch }&whereRv=${whereRv}'">이전</button>
				</c:if>
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<c:if test="${currentPage== i }">
						<button onclick="location.href='searchRvlist.re?pageNum=${i}&rvsearch=${rvsearch }&whereRv=${whereRv}'" disabled="disabled">${i}</button>
					</c:if>
					<c:if test="${currentPage!= i }">
						<button onclick="location.href='searchRvlist.re?pageNum=${i}&rvsearch=${rvsearch }&whereRv=${whereRv}'">${i}</button>
					</c:if>
				</c:forEach>
				<!-- 보여줄 것이 남아있다 -->
				<c:if test="${endPage < totalPage }">
					<button onclick="location.href='searchRvlist.re?pageNum=${endPage+1}&rvsearch=${rvsearch }&whereRv=${whereRv}'">다음</button>
				</c:if>
				</div>
				<div id="searchbox">
					<form action="searchRvlist.re">
						<select name="whereRv" id="whereRv"><option value="${whereRv}">${whereMS}</option><option value="id">아이디</option><option value="rvTitle">제목</option><option value="rvContent">내용</option> </select>
