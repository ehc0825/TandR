		...
		<div id="main_container">
			...
			<div id="pagebar" align="center">
				<c:if test="${startPage > PAGE_PER_BLOCK }">
					<button onclick="location.href='reviewMain.re?pageNum=${startPage-1}'">이전</button>
				</c:if>
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<c:if test="${currentPage== i }">
						<button onclick="location.href='reviewMain.re?pageNum=${i}'" disabled="disabled">${i}</button>
					</c:if>
					<c:if test="${currentPage!= i }">
						<button onclick="location.href='reviewMain.re?pageNum=${i}'">${i}</button>
					</c:if>
				</c:forEach>
				<!-- 보여줄 것이 남아있다 -->
				<c:if test="${endPage < totalPage }">
					<button onclick="location.href='reviewMain.re?pageNum=${endPage+1}'">다음</button>
				</c:if>
			<!-- num=0 처음 쓴글 -->
		</div>
		</div>
