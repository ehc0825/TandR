...
<form action="">
      <a id="category1" class="mymenu" href="movieClosed.mv"><u>상영 종료</u></a>
...
<td width="350px">제목: ${val.movieTitle }</td>
</tr>
<tr>
...
<tr>
<td></td>
<td width="70px"><a href="searchRvmainlist.re?whereRv=movieTitle&rvsearch=${val.movieTitle }" class="button">Review</a></td>
</tr>
<tr><br><br></tr>
</tbody>
</table>
...
<c:if test="${startPage>PAGE_PER_BLOK}">
 	<button onclick="location.href='movieClosed.mv?pageNum=${startPage-1}'">이전</button>
</c:if>
 <c:forEach var="i" begin="${startPage }" end="${endPage }">
<c:if test="${currentPage==i }">
 <button onclick="location.href='movieClosed.mv?pageNum=${i}'" disabled="disabled">${i }</button> 
</c:if>
	
 <c:if test="${currentPage!=i }">
 <button onclick="location.href='movieClosed.mv?pageNum=${i}'">${i }</button> 
</c:if>
 </c:forEach>
<c:if test="${currentPage < totalPage}">
						<button class="next" onclick="location.href='movieClosed.mv?pageNum=${currentPage + 1}'">
							다음
						</button>
					</c:if>
<c:if test="${endPage<totalPage }">
 
 		<button onclick="location.href='movieClosed.mv?pageNum=${endPage+1}'">다음</button>
</c:if>
</div>
</div>
</div>
