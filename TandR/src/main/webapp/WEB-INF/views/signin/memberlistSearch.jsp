...
<select name="whereM" id="whereM"><option value="${whereM}">${whereMS}</option><option value="id">아이디</option><option value="name">이름</option><option value="mailaddress">메일주소</option> </select>검색&nbsp;:&nbsp;<input type="text" id="Msearch" name="Msearch" value="${Msearch}"><input type="submit" value="검색"></form></div>
</div>
<div id="numberbar">
 <c:if test="${startPage>PAGE_PER_BLOK}">
 	<button onclick="location.href='searchMlist.si?pageNum=${startPage-1}&Msearch=${Msearch }&whereM=${whereM}'">이전</button>
</c:if>
 <c:forEach var="i" begin="${startPage }" end="${endPage }">
<c:if test="${currentPage==i }">
 <button onclick="location.href='searchMlist.si?pageNum=${i}&Msearch=${Msearch }&whereM=${whereM}'" disabled="disabled">${i }</button> 
</c:if>
	
 <c:if test="${currentPage!=i }">
 <button onclick="location.href='searchMlist.si?pageNum=${i}&Msearch=${Msearch }&whereM=${whereM}'">${i }</button> 
</c:if>
 </c:forEach>
<c:if test="${currentPage < totalPage}">
						<button class="next" onclick="location.href='searchMlist.si?pageNum=${currentPage + 1}&Msearch=${Msearch }&whereM=${whereM}'">
							다음
						</button>
					</c:if>
<c:if test="${endPage<totalPage }">
 
 		<button onclick="location.href='searchMlist.si?pageNum=${endPage+1}&Msearch=${Msearch }&whereM=${whereM}'">다음</button>
</c:if>
</div>
</div>
</section>
